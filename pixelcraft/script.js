...
function eventCard(event){
  ...
  return `<article class="event-card" data-event-id="${event.id}" style="--event-gradient:${event.gradient}">
    <div class="event-visual"><span class="event-code">${event.code} / ${event.category.toUpperCase()}</span><button class="save-event ${saved?'saved':''}" type="button" data-save="${event.id}" aria-label="${saved?'Remove':'Save'} ${event.title}">${saved?'◆':'◇'}</button><span class="event-glyph">${event.glyph}</span></div>
